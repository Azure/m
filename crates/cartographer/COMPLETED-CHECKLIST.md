# Completed checklist — `cartographer`

Append-only archive of completed checklist groups. See [CHECKLIST.md](CHECKLIST.md) for
any active work.

## Moved 2026-06-26 — observed-environment descriptor (D-CART-4), milestones EM-A..EM-E

Implemented the environment descriptor: an observed topology of **actors**, **roles**, and
**channels**, with roles as the substitution unit. Design: `D-CART-4` in
...
Re-planned 2026-06-26: callers are anonymous, so behavioral subdivision is a
**population-level** partition of a client role's traffic by operation (candidate child
roles the maintainer refines); transport/auth and identity axes deferred (identity gated on
PII-A). See D-CART-4.
