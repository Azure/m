...
//! Configuration model for the `squeaky` stress client.
//!
//! All execution parameters come from a JSON file (see [`Config::load`]). The
//! schema is intentionally shared in shape with the `talky` client so one config
//! file can drive either. Endpoints may be IPv4 or IPv6, and the
...
/// The `wordy` routes `squeaky` can drive.
...
impl Endpoint {
    ...
    pub fn parse(raw: &str) -> Result<Self, String> {
        ...
        let (host, port) = if let Some(after_bracket) = authority.strip_prefix('[') {
            ...
            (host.to_string(), parse_port(tail, default_port, raw)?)
        } else {
    ...
    /// The host rendered for a URL (IPv6 literals are bracketed).
    pub fn url_host(&self) -> String {
        match self.family {
            Family::V6 => format!("[{}]", self.host),
            _ => self.host.clone(),
        }
    }

    /// A base URL `scheme://host:port` (no path).
    pub fn base_url(&self) -> String {
        format!("{}://{}:{}", self.scheme_str(), self.url_host(), self.port)
    }
}
...
mod tests {
    ...
    #[test]
    fn parses_ipv6_endpoint_and_brackets_url_host() {
        let e = Endpoint::parse("http://[::1]:8080").unwrap();
        assert_eq!(e.host, "::1");
        assert_eq!(e.port, 8080);
        assert_eq!(e.family, Family::V6);
        assert_eq!(e.url_host(), "[::1]");
        assert_eq!(e.base_url(), "http://[::1]:8080");
    }

    #[test]
    fn defaults_ports_by_scheme() {
        assert_eq!(Endpoint::parse("http://host").unwrap().port, 80);
        let https = Endpoint::parse("https://host").unwrap();
        assert_eq!(https.port, 443);
        assert_eq!(https.scheme_str(), "https");
    }

    #[test]
    fn classifies_hostnames() {
        assert_eq!(
            Endpoint::parse("http://localhost:9").unwrap().family,
            Family::Name
        );
    }
    ...
    fn select_endpoints_applies_filter() {
        ...
        assert_eq!(selected[0].family, Family::V4);
    }

    #[test]
    fn validate_requires_a_bound() {
        let config: Config =
            serde_json::from_str(r#"{ "endpoints": ["http://127.0.0.1:8080"] }"#).unwrap();
    ...
    fn rejects_unknown_fields() {
        assert!(serde_json::from_str::<Config>(r#"{ "endpoints": [], "bogus": 1 }"#).is_err());
    }
}
