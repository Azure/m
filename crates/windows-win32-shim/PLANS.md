...
|---|---|---|---|
| [CHECKLIST-offproc.md](CHECKLIST-offproc.md) | in progress | Move interception-response work off the calling thread (then out of process) via marshaled JSON work items + a `WaitOnAddress` latch. Milestones OT (off-thread synchronous), BC (bounded, compact bodies), RS (panic-safe remoting), and UT (untranscoded, encoding-tagged text — api-journal → shim → cartographer) **complete**; milestone AC (async, zero-copy capture for async-capable seams) **planned**. | [DESIGN-NOTES.md](DESIGN-NOTES.md) (SHIM-D25, SHIM-D26, SHIM-D27) |
