...
|---|---|---|---|
| [CHECKLIST-offproc.md](CHECKLIST-offproc.md) | in progress | Move interception-response work off the calling thread (then out of process) via marshaled JSON work items + a `WaitOnAddress` latch. Milestone OT (off-thread synchronous) **complete** — deferred stages (true async, out-of-process collector, response mutation) remain. | [DESIGN-NOTES.md](DESIGN-NOTES.md) (SHIM-D25) |
