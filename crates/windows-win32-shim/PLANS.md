...
|---|---|---|---|
| [CHECKLIST-offproc.md](CHECKLIST-offproc.md) | in progress | Move interception-response work off the calling thread (then out of process) via marshaled JSON work items + a `WaitOnAddress` latch. Milestones OT (off-thread synchronous) and BC (bounded, compact bodies) **complete**; milestone UT (untranscoded, encoding-tagged text — spans api-journal → shim → cartographer) **planned**. | [DESIGN-NOTES.md](DESIGN-NOTES.md) (SHIM-D25, SHIM-D26) |
