...
mod tests {
    ...
    fn real_manifest_active_and_aliased_counts() {
        ...
        // Active exports today: W forms + NOT_SUPPORTED stubs + the 10 registry A
        // forms (MW6-2) + the 11 filesystem A forms (MW6-3) + the 10 loader-family
        // exports (MW9) + the 6 COM exports (MW10) + the 1 web-host activation
        // export `mRegisterModule` (MW11) + the 12 WinHTTP egress exports (MW17).
        // mCloseHandle is the sole noalias export, so one fewer is aliased.
        assert_eq!(m.exports.len(), 112);
        assert_eq!(m.aliased.len(), 111);
    }
