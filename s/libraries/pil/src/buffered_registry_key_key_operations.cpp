...
namespace m::pil::impl::buffered
{
    ...
    {
        using std::swap;
        swap(m_underlying_key, other.m_underlying_key);
        swap(m_keys, other.m_keys);
        swap(m_values, other.m_values);
        swap(m_last_write_time, other.m_last_write_time);
        swap(m_security_descriptor, other.m_security_descriptor);
    }
    ...
    key::initialize_keys_overlay()
    {
        ...
        std::array<std::u16string, 32>                 key_array;
        std::span<std::u16string, std::dynamic_extent> key_span{key_array};
        std::size_t                                    index{};
                    ...
                    std::shared_ptr<ikey>& returned_key)
    {
        ...
        auto [insertion_it, inserted] =
            m_keys.emplace(std::make_pair(key_name,
        ...
        if (node.m_deleted)
        {
            ...
            node.m_key     = std::make_shared<key>();
    ...
    key::delete_key(ikey::delete_key_flags flags, std::u16string_view key_name, sam)
    {
        ...
        auto const find_result = m_keys.find(key_name);
                  ...
                  std::shared_ptr<ikey>&             returned_key)
    {
        ...
        auto find_iter = m_keys.find(key_name.value());
    ...
    key::rename_key(ikey::rename_key_flags             flags,
                    ...
                    std::u16string_view                new_key_name)
    {
        auto const entry_time = time_point::clock::now();
        ...
        if (old_key_name_value.contains(uregistry_delimiter))
            throw m::invalid_parameter("ikey::rename_key.old_key_name");

        if (new_key_name.contains(uregistry_delimiter))
            throw m::invalid_parameter("ikey::rename_key.new_key_name");

        auto lock = std::unique_lock(m_mutex);

        auto const find_iter = m_keys.find(old_key_name_value);
        ...
        auto const [new_it, succeeded] = m_keys.emplace(new_key_name, new_node);
    ...
    bool
    key::is_subkey_empty(key_name_view_type key_name)
    {
        auto const it = m_keys.find(key_name);
        ...
        key_name_type                                 subkey_name;
        std::span<key_name_type, std::dynamic_extent> subkey_name_span(&subkey_name, 1);
    ...
    void
    key::unmirror_node(key_name_view_type key_name, key_node& node)
    {
        std::ignore = key_name;
        std::ignore = node;
        // to do
