...
namespace m::win32
{
    event::event(create_event_flags flags, DWORD desired_access): m_event_kind{event_kind::none}
    {
        event temp;
    ...
    event::event(event_kind kind, DWORD desired_access): m_event_kind{event_kind::none}
    {
        event temp;
    ...
    event::reset()
    {
        auto old_handle = std::exchange(m_handle, nullptr);
        auto old_kind   = std::exchange(m_event_kind, event_kind::none);

        std::ignore = old_kind;

        handle::close_handle(old_handle);
    }
