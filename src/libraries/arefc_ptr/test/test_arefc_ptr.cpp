...
TEST(AreFcPtr_CAS, SuccessUpdatesTarget)
{
    auto p1 = m::mmake_arefc<Plain>();
    p1->value = 1;
    auto p2 = m::mmake_arefc<Plain>();
    p2->value = 2;

    m::arefc_ptr<Plain> target   = p1;
    m::arefc_ptr<Plain> expected = p1;

    // target == expected, so CAS should swap target to p2.
    bool ok = target.compare_exchange_strong(expected, p2);

    EXPECT_TRUE(ok);
    EXPECT_EQ(target.get(), p2.get());
    // On success, `expected` is not modified.
    EXPECT_EQ(expected.get(), p1.get());
}

TEST(AreFcPtr_CAS, SuccessNoRefLeak)
{
    // Verify that a successful CAS leaves every object with the correct
    // final refcount — none should linger when their last holder is dropped.
    LifetimeTracker::s_live_count = 0;
    {
        auto p1 = m::mmake_arefc<LifetimeTracker>(1);
        auto p2 = m::mmake_arefc<LifetimeTracker>(2);
        ...
        m::arefc_ptr<LifetimeTracker> target   = p1;
        m::arefc_ptr<LifetimeTracker> expected = p1;
        ...
        target.reset();
        expected.reset();
        EXPECT_EQ(LifetimeTracker::s_live_count, 2); // p1 and p2 still held by locals
    }
    EXPECT_EQ(LifetimeTracker::s_live_count, 0);
}

// ============================================================================
// compare_exchange_strong — failure case
// ============================================================================

TEST(AreFcPtr_CAS, FailureUpdatesExpected)
{
    auto p1 = m::mmake_arefc<Plain>();
    p1->value = 1;
    auto p2 = m::mmake_arefc<Plain>();
    p2->value = 2;
    auto p3 = m::mmake_arefc<Plain>();
    p3->value = 3;

    m::arefc_ptr<Plain> target   = p3; // holds p3
    m::arefc_ptr<Plain> expected = p1; // but we think it holds p1 — mismatch

    bool ok = target.compare_exchange_strong(expected, p2);

    EXPECT_FALSE(ok);
    EXPECT_EQ(target.get(), p3.get());   // target unchanged
    EXPECT_EQ(expected.get(), p3.get()); // expected updated to actual current value
}

TEST(AreFcPtr_CAS, FailureNoRefLeakOnDesired)
{
    // This test exposes a ref-count leak bug that was present on the CAS failure
    // path: increment_ref(desired) was called unconditionally before the CAS,
    // but if the CAS failed, the increment was never balanced.
    //
    // After the fix (decrement_ref(d) on the failure path), `desired`'s refcount
    // must return to exactly 1 (held only by the local `p2` variable) so that
    // when `p2` drops at the end of the scope, the object is destroyed.
    LifetimeTracker::s_live_count = 0;
    {
        auto p1 = m::mmake_arefc<LifetimeTracker>(1);
        auto p2 = m::mmake_arefc<LifetimeTracker>(2); // this is `desired`
        auto p3 = m::mmake_arefc<LifetimeTracker>(3);
        EXPECT_EQ(LifetimeTracker::s_live_count, 3);

        m::arefc_ptr<LifetimeTracker> target   = p3;
        m::arefc_ptr<LifetimeTracker> expected = p1;

        bool ok = target.compare_exchange_strong(expected, p2);
        EXPECT_FALSE(ok);

        // Drop the CAS temporaries so they hold no extra references.
        target.reset();
        expected.reset();

        // Still 3 live objects — p1, p2, p3 each held by one local variable.
        EXPECT_EQ(LifetimeTracker::s_live_count, 3);
    }
    // All locals dropped — all 3 objects must be destroyed.
    // If the ref-count leak is present, p2 will not be destroyed here and
    // s_live_count will remain 1.
    EXPECT_EQ(LifetimeTracker::s_live_count, 0);
}
