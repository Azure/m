...
{
    std::array const byte_array{std::byte{'a'}, std::byte{'b'}, std::byte{'c'}};
    auto const       byte_span = std::span(byte_array);

    auto const us = m::unique_span(byte_span);

    EXPECT_EQ(us.size(), byte_span.size());
    EXPECT_TRUE(std::ranges::equal(byte_span, us));
}
...
TEST(MemoryUniqueSpan, ByteSpan2)
{
    ...
    auto const us = m::unique_span(byte_array);

    EXPECT_EQ(us.size(), byte_array.size());
    EXPECT_TRUE(std::ranges::equal(byte_array, us));
}
...
bool
operator==(owc const& l, owc const& r)
{
    return l.m_b == r.m_b;
}
