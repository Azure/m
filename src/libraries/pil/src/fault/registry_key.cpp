...
{
    key::key(std::shared_ptr<ikey> const& underlying_key, std::shared_ptr<fault_script> const& script):
        m_key(underlying_key), m_script(script)
    {
    ...
    {
        m_script->check(fault_operation::create_key, ikey::get_path() + key_path);
        ...
        if (unmapped_returned_key)
            returned_key = std::make_shared<key>(unmapped_returned_key, m_script);
        return d;
    }
    ...
    {
        m_script->check(fault_operation::delete_key, ikey::get_path() + key_path);
    ...
    {
        m_script->check(fault_operation::delete_tree, ikey::get_path() + key_path);
    ...
    key::open_key(ikey::open_key_flags                flags,
                  ...
                  std::error_code&                    ec)
    {
        m_script->check(fault_operation::open_key, ikey::get_path() + key_path);

        std::shared_ptr<ikey> temp_key;
        auto                  d = m_key->open_key(flags, key_path, sam_desired, temp_key, ec);
        if (temp_key)
            returned_key = std::make_shared<key>(temp_key, m_script);
        return d;
    }
    ...
    {
        m_script->check(fault_operation::rename_key, ikey::get_path() + sub_key_name);
    ...
    {
        m_script->check(fault_operation::delete_value, ikey::get_path(), name.view());
    ...
    {
        m_script->check(fault_operation::get_value, ikey::get_path(), value_name.view());
    ...
    {
        m_script->check(fault_operation::set_value, ikey::get_path(), value_name.view());
