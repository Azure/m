...
{
    //
    // file: reads pass straight through; a file node has no mutating verbs.
    //

    file::file(std::shared_ptr<ifile> const& underlying_file, std::shared_ptr<log> const& log_ptr):
        m_file(underlying_file), m_log(log_ptr)
    {
    ...
    //
    // directory: mutations are recorded with the requested-vs-done shape and
    // then forwarded; returned nodes are re-wrapped so the whole subtree stays
    // inside the logging layer. Reads forward unchanged.
    //

    directory::directory(std::shared_ptr<idirectory> const& underlying_directory,
                         std::shared_ptr<log> const&        log_ptr):
        m_directory(underlying_directory), m_log(log_ptr)
    {
        M_INTERNAL_ERROR_CHECK(m_log.get() != nullptr);
    }
    ...
    {
        auto logentry = std::make_unique<fs_create_directory_log_entry>(flags, path, access);
        std::shared_ptr<idirectory> unwrapped;
        auto const d = m_directory->create_directory(flags, path, access, unwrapped);
        if (unwrapped)
            returned_directory = std::make_shared<directory>(unwrapped, m_log);
    ...
    {
        auto logentry = std::make_unique<fs_create_file_log_entry>(flags, path, access);
        std::shared_ptr<ifile> unwrapped;
        auto const             d = m_directory->create_file(flags, path, access, unwrapped);
        if (unwrapped)
            returned_file = std::make_shared<file>(unwrapped, m_log);
        logentry->set_disposition(d);
        m_log->add(logentry);
        return d;
    }
    ...
    {
        auto logentry = std::make_unique<fs_remove_entry_log_entry>(flags, name);
    ...
    {
        auto logentry = std::make_unique<fs_delete_tree_log_entry>(flags, name);
    ...
    {
        auto logentry = std::make_unique<fs_rename_entry_log_entry>(flags, old_path, new_path);
    ...
    //
    // filesystem: open_root is a read; forward and re-wrap the returned root.
    //

    filesystem::filesystem(std::shared_ptr<ifilesystem> const& underlying_filesystem,
                           std::shared_ptr<log> const&         log_ptr):
        m_filesystem(underlying_filesystem), m_log(log_ptr)
    {
        M_INTERNAL_ERROR_CHECK(m_log.get() != nullptr);
    }
    ...
    filesystem::initialize_monitor(m::locked_t)
    {
        ...
        // Monitoring is a read-side capability and is not logged; forward the
        // underlying monitor through a transparent wrapper.
