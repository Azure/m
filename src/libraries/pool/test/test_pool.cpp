...
{
    auto p = std::make_shared<m::pool<m::inplace_vector<char8_t, 1024>, 512>>();
...
{
    using pool_type       = m::pool<char, 8>;
    using unique_ptr_type = pool_type::unique_ptr_type;

    auto                         p = std::make_shared<pool_type>();
    std::vector<unique_ptr_type> allocated;
...
{
    auto p = std::make_shared<m::pool<int, 2>>();
    auto a = p->allocate();
    auto b = p->allocate();
...
{
    auto pool_ptr = std::make_shared<m::pool<double, 4>>();
    auto obj1     = pool_ptr->allocate();
    auto obj2     = pool_ptr->allocate();
    ...
    *obj2 = 2.71;
    ...
    auto obj3 = pool_ptr->allocate();
    ASSERT_NE(obj3.get(), nullptr);
}

TEST(TestPool, PoolExhaustionBlocks)
{
    auto p   = std::make_shared<m::pool<int, 1>>();
    auto obj = p->allocate();
