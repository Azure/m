...
namespace m::threadpool_impl
{
    timer::timer(timer::task_type&& task, std::wstring&& description):
        m_task(std::move(task)), m_duration{}, m_description(std::move(description))
    {
    ...
    void
    timer::compute_timer_times(duration dur, set_threadpool_timer_ex_parameters& parameters)
    {
