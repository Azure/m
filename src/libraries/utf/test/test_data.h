...
struct utf_data_set
{
    utf_data_set(std::initializer_list<char8_t>  utf8data,
        ...
        m_u8chardata(utf8data.begin(), utf8data.end()),
        m_u16chardata(utf16data.begin(), utf16data.end()),
        m_u32chardata(utf32data.begin(), utf32data.end())
    {
        m_u8sv  = std::u8string_view(&m_u8chardata[0], m_u8chardata.size());
        m_u16sv = std::u16string_view(&m_u16chardata[0], m_u16chardata.size());
        m_u32sv = std::u32string_view(&m_u32chardata[0], m_u32chardata.size());
    }

    // Not using string because the data may be invalid and we don't want string-ish
    // interpretation of the data ever. These are sequences of char8_ts and char32_ts.
    std::vector<char8_t>  m_u8chardata;
    std::vector<char16_t> m_u16chardata;
    std::vector<char32_t> m_u32chardata;

    std::u8string_view  m_u8sv;
    std::u16string_view m_u16sv;
    std::u32string_view m_u32sv;
};
